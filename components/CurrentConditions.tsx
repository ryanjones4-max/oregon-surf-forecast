...
import { getWeatherLabel, getWeatherEmoji } from '@/lib/weatherCodes'
...
function ConditionCard({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="rounded-lg border border-sl-border bg-sl-card p-3">
      <div className="mb-1.5 text-[10px] font-semibold uppercase tracking-widest text-sl-muted">{label}</div>
      {children}
    </div>
  )
}
...
export function CurrentConditions({ forecast: fc, lat, lng }: Props) {
  ...
  return (
    <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-1 lg:gap-2.5">
      ...
      {/* Swell */}
      <ConditionCard label="Swell">
        <div className="text-sm font-semibold text-white">
          {metersToFeet(fc.swellHeight).toFixed(1)}ft @ {fc.swellPeriod.toFixed(0)}s
        </div>
        <div className="mt-0.5 flex items-center gap-1 text-xs text-sl-muted">
          <svg viewBox="0 0 24 24" className="h-3.5 w-3.5" style={{ transform: `rotate(${fc.swellDirection}deg)` }}>
      ...
      {/* Wind */}
      {fc.windSpeed != null && (
        <ConditionCard label="Wind">
          <div className="text-sm font-semibold text-white">
            {kmhToMph(fc.windSpeed).toFixed(0)} mph
          </div>
          <div className="mt-0.5 flex items-center gap-1 text-xs text-sl-muted">
            <svg viewBox="0 0 24 24" className="h-3.5 w-3.5" style={{ transform: `rotate(${fc.windDirection ?? 0}deg)` }}>
              <path d="M12 2 L18 14 L12 10 L6 14 Z" fill="currentColor" />
            </svg>
      ...
      {/* Temperature */}
      <ConditionCard label="Temperature">
      ...
      {/* Weather — links to NWS forecast for the area */}
      {fc.weatherCode != null && (
        <a
          href={lat != null && lng != null ? `https://forecast.weather.gov/MapClick.php?lat=${lat}&lon=${lng}` : 'https://forecast.weather.gov'}
          target="_blank"
          rel="noopener noreferrer"
          className="group rounded-lg border border-sl-border bg-sl-card p-3 transition-colors hover:border-sl-accent/40 hover:bg-sl-surface"
        >
          ...
          <div className="flex items-center gap-2">
            <span className="text-lg">{getWeatherEmoji(fc.weatherCode)}</span>
